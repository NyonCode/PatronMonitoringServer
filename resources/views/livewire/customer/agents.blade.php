<div class="space-y-6 p-4 md:p-6" wire:poll.500s>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr>
                    ...
                    <th class="px-1 md:px-2 lg:px-4 2xl:px-6 py-1 lg:py-3 text-left">
                        <span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">IP adresa</span>
                    </th>
                    ...
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
                        ...
                        <td class="px-1 md:px-2 lg:px-4 2xl:px-6 py-2 lg:py-4">
                            <div>
                                ...
                                @endif
                                <!-- Badges for pending commands and active terminals -->
                                <div class="flex gap-1 mt-1">
                                    @if($agent->pending_commands_count > 0)
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300">
                                            {{ $agent->pending_commands_count }} cmd
                                        </span>
                                    @endif
                                    @if($agent->active_terminals_count > 0)
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">
                                            {{ $agent->active_terminals_count }} term
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <td class="px-1 md:px-2 lg:px-4 2xl:px-6 py-2 lg:py-4">
                            <div class="flex items-center justify-center gap-1 flex-wrap">
                                <button wire:click="showDetail({{ $agent->id }})"
                                        class="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded transition-colors"
                                        title="Detail">
                                ...
                                <button wire:click="showCommands({{ $agent->id }})"
                                        class="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition-colors"
                                        title="Příkazy">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button wire:click="showTerminal({{ $agent->id }})"
                                        class="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/30 rounded transition-colors"
                                        title="Terminál">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                    </svg>
    ...
    @if($showCommandsModal && $selectedAgentId)
        @livewire('customer.agent-commands', ['agent' => $this->agents->find($selectedAgentId)], key('agent-commands-'.$selectedAgentId))
    @endif

    @if($showTerminalModal && $selectedAgentId)
        @livewire('customer.agent-terminal', ['agent' => $this->agents->find($selectedAgentId)], key('agent-terminal-'.$selectedAgentId))
    @endif
</div>

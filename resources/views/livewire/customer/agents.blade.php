            ...
            <table class="w-full">
                ...
                <tr>
                    <th class="px-4 py-3 text-left">
                        <button wire:click="sortBy('hostname')" class="flex items-center gap-2 font-semibold text-sm text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100">
                            Název
                            @if($sortBy === 'hostname')
                    ...
                    <th class="px-4 py-3 text-left"><span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">Status</span></th>
                    <th class="px-4 py-3 text-left">
                        <button wire:click="sortBy('ip_address')" class="flex items-center gap-2 font-semibold text-sm text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100">
                            IP adresa
                    ...
                    <th class="px-4 py-3 text-left"><span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">CPU</span></th>
                    <th class="px-4 py-3 text-left"><span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">RAM</span></th>
                    <th class="px-4 py-3 text-left"><span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">GPU</span></th>
                    <th class="px-4 py-3 text-left"><span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">Disk</span></th>
                    <th class="px-4 py-3 text-center"><span class="font-semibold text-sm text-zinc-700 dark:text-zinc-300">Akce</span></th>
                </tr>
                </thead>
                ...
                @forelse($this->agents as $agent)
                    ...
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
                        ...
                        <!-- Desktop table content (zkráceno pro přehlednost) -->
                        <td class="px-4 py-4">
                            <div>
                                <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $name }}</div>
                                @if(!empty($agent->pretty_name))
                                    <div class="text-sm text-zinc-500 dark:text-zinc-400">{{ $agent->hostname }}</div>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-4">
                            ...
                            @elseif($status === 'shutdown')
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-900/30 text-gray-800 dark:text-gray-300">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <td class="px-4 py-4">
                            <span class="text-sm text-zinc-700 dark:text-zinc-300 font-mono">{{ $agent->ip_address ?? 'N/A' }}</span>
                        </td>
                        <!-- Metriky CPU/RAM/GPU -->
                        <td class="px-4 py-4">
                            <div class="space-y-1 min-w-[100px]">
                                <div class="flex items-center justify-end">
                                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-semibold transition-colors"
                                          :class="{
                            ...
                            @if($disk)
                                <div class="space-y-1 min-w-[130px]">
                                    <div class="flex items-center justify-between gap-2">
                                        <span class="text-xs text-zinc-600 dark:text-zinc-400">{{ $disk['name'] }}</span>
                                        <span class="inline-flex px-2 py-0.5 rounded text-xs font-semibold
                                            @if($disk['usage_percent'] > 90) bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300
                                            @elseif($disk['usage_percent'] > 75) bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300
                                    ...
                                    <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ $disk['free'] }} / {{ $disk['size'] }}</div>
                                </div>
                            @else
                        ...
                        <td class="px-4 py-4">
                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.outside="open = false" x-transition
                                     class="absolute right-0 mt-2 w-48 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-xl z-50">
                                    <button wire:click="showDetail({{ $agent->id }})" class="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors">
                                    ...
                                    <button wire:click="showLog({{ $agent->id }})" class="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                        </svg>
                                        {{ __('Log') }}
                                    </button>
                                    <button disabled wire:click="showConfig({{ $agent }})" class="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-zinc-400 dark:text-zinc-600 cursor-not-allowed">
                                    ...
                                    <button wire:click="showDelete({{ $agent }})" class="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                                        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
        ...
        @forelse($this->agents as $agent)
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 p-4"
                ...
                <div class="grid grid-cols-3 gap-2 mb-3">
                    ...
                    <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-2">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300" x-text="cpu + '%'"></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-2">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300" x-text="ram + '%'"></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-2">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300" x-text="gpu + '%'"></span>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="showDetails" x-collapse>
                    @if($disk)
                        <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-3 mb-3">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-medium text-zinc-700 dark:text-zinc-300">Disk: {{ $disk['name'] }}</span>
                                <span class="inline-flex px-2 py-0.5 rounded text-xs font-semibold
                                    @if($disk['usage_percent'] > 90) bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300
                                    @elseif($disk['usage_percent'] > 75) bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300
                                    @else bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 @endif">
                            ...
                            <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ $disk['free'] }} volných z {{ $disk['size'] }}</div>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-2 pt-3 border-t border-zinc-200 dark:border-zinc-700">
                    <button @click="showDetails = !showDetails" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 text-xs font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-lg transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" x-show="!showDetails">
            ...
            <div class="flex items-center gap-2 pt-3" x-show="showDetails">
                <button disabled wire:click="showConfig({{ $agent }})" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 text-xs font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-lg transition-colors">
                    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M10.343 3.94c.09-.542.56-.94 1.11-.94h1.093c.55 0 1.02.398 1.11.94l.149.894c.07.424.384.764.78.93.398.164.855.142 1.205-.108l.737-.527a1.125 1.125 0 0 1 1.45.12l.773.774c.39.389.44 1.002.12 1.45l-.527.737c-.25.35-.272.806-.107 1.204.165.397.505.71.93.78l.893.15c.543.09.94.559.94 1.109v1.094c0 .55-.397 1.02-.94 1.11l-.894.149c-.424.07-.764.383-.929.78-.165.398-.143.854.107 1.204l.527.738c.32.447.269 1.06-.12 1.45l-.774.773a1.125 1.125 0 0 1-1.449.12l-.738-.527c-.35-.25-.806-.272-1.203-.107-.398.165-.71.505-.781.929l-.149.894c-.09.542-.56.94-1.11.94h-1.094c-.55 0-1.019-.398-1.11-.94l-.148-.894c-.071-.424-.384-.764-.781-.93-.398-.164-.854-.142-1.204.108l-.738.527c-.447.32-1.06.269-1.45-.12l-.773-.774a1.125 1.125 0 0 1-.12-1.45l.527-.737c.25-.35.272-.806.108-1.204-.165-.397-.506-.71-.93-.78l-.894-.15c-.542-.09-.94-.56-.94-1.109v-1.094c0-.55.398-1.02.94-1.11l.894-.149c.424-.07.765-.383.93-.78.165-.398.143-.854-.108-1.204l-.526-.738a1.125 1.125 0 0 1 .12-1.45l.773-.773a1.125 1.125 0 0 1 1.45-.12l.737.527c.35.25.807.272 1.204.107.397-.165.71-.505.78-.929l.15-.894Z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                    </svg>
                ...
                <button wire:click="showDelete({{ $agent }})" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 text-xs font-medium text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-800 hover:bg-red-200 dark:hover:bg-red-700 rounded-lg transition-colors">
                    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                    </svg>
                    {{ __('agents.delete') }}
